...
public final class Table extends Observable {
    ...
    private Color lightSquareColor = Color.decode("#FFFACD");
    private Color darkSquareColor = Color.decode("#593E1A");
    ...
    private JMenu createPreferencesMenu() {
        ...
        final JMenuItem chooseDarkMenuItem = new JMenuItem("Choose Dark Square Color");
        colorChooserSubMenu.add(chooseDarkMenuItem);

        final JMenuItem chooseLightMenuItem = new JMenuItem("Choose Light Square Color");
        ...
        chooseDarkMenuItem.addActionListener(e -> {
            final Color colorChoice = JColorChooser.showDialog(Table.get().getGameFrame(), "Choose Dark Square Color",
                    Table.get().getGameFrame().getBackground());
            if (colorChoice != null) {
                Table.get().getBoardPanel().setSquareLightColor(chessBoard, colorChoice);
            }
        });

        chooseLightMenuItem.addActionListener(e -> {
            final Color colorChoice = JColorChooser.showDialog(Table.get().getGameFrame(), "Choose Light Square Color",
                    Table.get().getGameFrame().getBackground());
            if (colorChoice != null) {
                Table.get().getBoardPanel().setSquareLightColor(chessBoard, colorChoice);
            }
    ...
    private class BoardPanel extends JPanel {
        ...
        void drawBoard(final Board board) {
            removeAll();
            for (final SquarePanel boardSquare : boardDirection.traverse(boardSquares)) {
                boardSquare.drawSquare(board);
                add(boardSquare);
            }
            validate();
            repaint();
        ...
        void setSquareDarkColor(final Board board,
                                 final Color darkColor) {
            for (final SquarePanel boardSquare : boardSquares) {
                boardSquare.setDarkSquareColor(darkColor);
            }
            drawBoard(board);
        }

        void setSquareLightColor(final Board board,
                                 final Color lightColor) {
            for (final SquarePanel boardSquare : boardSquares) {
                boardSquare.setLightSquareColor(lightColor);
            }
            drawBoard(board);
        }
    ...
    private class SquarePanel extends JPanel {
        ...
        SquarePanel(final BoardPanel boardPanel,
            ...
            highlightSquareBorder(chessBoard);
                ...
                public void mouseClicked(final MouseEvent event) {
                    ...
                    if (isRightMouseButton(event)) {
                        ...
                        if (sourceSquare == null) {
                            sourceSquare = chessBoard.getPiece(squareID);
                            humanMovedPiece = sourceSquare;
                            if (humanMovedPiece == null) {
                                sourceSquare = null;
                            }
                        } else {
                            final Move move = Move.MoveFactory.createMove(chessBoard, sourceSquare.getPiecePosition(),
                                    squareID);
        ...
        void setLightSquareColor(final Color color) {
            lightSquareColor = color;
        }

        void setDarkSquareColor(final Color color) {
            darkSquareColor = color;
        }

        private void highlightSquareBorder(final Board board) {
            if(humanMovedPiece != null &&
                    humanMovedPiece.getPieceColor() == board.currentPlayer().getColor() &&
                    humanMovedPiece.getPiecePosition() == this.squareID) {
        ...
        private void highlightAIMove() {
            if(computerMove != null) {
                if(this.squareID == computerMove.getCurrentCoordinate()) {
                    setBackground(Color.pink);
                } else if(this.squareID == computerMove.getDestinationCoordinate()) {
        ...
        private void assignSquarePieceIcon(final Board board) {
            this.removeAll();
            if(board.getPiece(this.squareID) != null) {
                try{
                    final BufferedImage image = ImageIO.read(new File(pieceIconPath +
                            board.getPiece(this.squareID).getPieceColor().toString().substring(0, 1) + "" +
                            board.getPiece(this.squareID).toString() +
                            ".gif"));
        ...
        private void assignSquareColor() {
            if (BoardUtils.INSTANCE.FIRST_ROW.get(this.squareID) ||
                    BoardUtils.INSTANCE.THIRD_ROW.get(this.squareID) ||
                    BoardUtils.INSTANCE.FIFTH_ROW.get(this.squareID) ||
                    BoardUtils.INSTANCE.SEVENTH_ROW.get(this.squareID)) {
                setBackground(this.squareID % 2 == 0 ? lightSquareColor : darkSquareColor);
            } else if(BoardUtils.INSTANCE.SECOND_ROW.get(this.squareID) ||
                    BoardUtils.INSTANCE.FOURTH_ROW.get(this.squareID) ||
                    BoardUtils.INSTANCE.SIXTH_ROW.get(this.squareID)  ||
                    BoardUtils.INSTANCE.EIGHTH_ROW.get(this.squareID)) {
                setBackground(this.squareID % 2 != 0 ? lightSquareColor : darkSquareColor);
            }
        }
    }
